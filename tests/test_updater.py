...
class UpdaterTests(unittest.TestCase):
    def test_only_newer_stable_versions_are_offered(self):
        ...
        major, minor, patch = version_tuple(APP_VERSION)
        self.assertTrue(is_newer(f"v{major}.{minor}.{patch + 1}", APP_VERSION))
        self.assertFalse(is_newer(f"v{APP_VERSION}", APP_VERSION))
        self.assertFalse(is_newer("v19.1.1", APP_VERSION))
        self.assertFalse(is_newer(f"v{APP_VERSION}-rc1", APP_VERSION))
